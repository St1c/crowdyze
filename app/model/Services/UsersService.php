...
class UsersService extends Nette\Object
{

	/** @var usersRepository */
	private $usersRepository;
	/** @var users_detailsRepository */
	private $users_detailsRepository;
	...
	public function __construct(Repositories\UsersRepository $usersRepository,
								Repositories\Users_detailsRepository $users_detailsRepository,
								...
								Utilities\MailerService $mailerService )
	{
		...
		$this->users_detailsRepository 	= $users_detailsRepository;
	...
	{
		return $this->usersRepository->find($by);
	}

	public function update(ActiveRow $user, array $data)
	{
		return $user->related('users_details')->update($data);
	}
	...
	{
		return $this->usersRepository->find(array('id' => $id));
	}
	...
	{
		$user = $this->usersRepository->find(array('email' => $data['email']));
	...
	{
		return $this->usersRepository->create(array(
			'email' 		=> $me['email'],
			'role' 			=> 'user',
			'active'		=> $setActive,
		));
	...
	{
		return $user->related('users_details')->insert(array(
			'facebook_id' 	=> $me['id'],
	...
	{
		$updateData = array();
		$users_details = $user->related('users_details')->fetch();
		...
		if ( empty($users_details->facebook_id) ) {
		...
		if ( empty($users_details->profile_photo) ) {
		...
		// Update changes
		if (!empty($updateData)) {
			$this->users_detailsRepository->update($users_details, $updateData);
		}
	}
	...
	private function updateFromGoogle($user, array $me)
	{
		...
		if ( empty($users_details->google_id) ) {
	...
	{
		return $this->accepted_tasksRepository->getAcceptedByUser($userId);
	}
	...
	{
		return $this->accepted_tasksRepository->getFinishedByUser($userId);
	}
	...
	{
		return $this->accepted_tasksRepository->getSatisfiedByUser($userId);
	}
	...
	{
		return $this->accepted_tasksRepository->getUnsatisfiedByUser($userId);
	}
	...
	{
		return $this->usersRepository->createIdentity($user);
	}
